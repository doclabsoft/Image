...
prototype.getBEMClass = function(component, elClass, modClass) {
  ...
  elClass = elClass ? '__'+elClass : '';
  modClass = modClass ? '_'+modClass : '';
  return prefix + elClass + modClass;
};
...
prototype.createDom = function(component) {
  ...
  var customClass = component.getCssClass();
  if (customClass)
    classes.push(customClass);

  var element = dom.createDom(this.getTagName(component), classes.join(' '));
 ...
 * @protected
 */
prototype.getTagName = function(component) {
  return component.getTagName() || this.tagName;
};
...
prototype.initializeDom = function(component) {
  ...
  this.updateDomListeners(component);
};
...
prototype.uninitializeDom = function(component) {
  ...
  component.$cache('domListeners', null);
};

prototype.updateDomListeners = function(component) {
  ...
  var oldListeners = component.$cache('domListeners');
  if (oldListeners) {
    var i, data;
    for (i=0; i<oldListeners.length; i++) {
      data = oldListeners[i];
      eventHandler.unlisten(element, data.type, data.listener, data.capture, data.scope);
    }
  }

  var newListeners = component.getDomListeners();
  if (newListeners && newListeners.length) {
    var i, data;
    for (i=0; i<newListeners.length; i++) {
      data = newListeners[i];
      eventHandler.listenWithScope(element, data.type, data.listener, data.capture, data.scope);
    }
  }
  component.$cache('domListeners', Array.prototype.slice.call(newListeners));
};
...
prototype.getStates = function(element) {
  ...
  if (classNames) {
    goog.array.forEach(classNames, function(className) {
      ...
      if (className) {
        var parsedState = parseInt(this.getStateByClass(className), 10);
